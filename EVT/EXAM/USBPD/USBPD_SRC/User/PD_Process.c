...
void PD_SRC_Init( void )
{
    ...
    EXTEN->CTLR2|=EXTEN_CC1_REF;
    USBPD->PORT_CC1 &= ~CC_CVS_Mask;
#if(CC_2==CC2)
    EXTEN->CTLR2|=EXTEN_CC2_REF;
    USBPD->PORT_CC2 &= ~CC_CVS_Mask;
#elif(CC_2==CC3)
    EXTEN->CTLR2|=EXTEN_CC3_REF;
...
#elif(CC_2==CC3)
    USBPD->PORT_CC3 &= ~CC_PU_80;
#endif
    USBPD->PORT_CC1 = CC_CE | CC_CVS_66 | CC_PU_330;
#if(CC_2==CC2)
    USBPD->PORT_CC2 = CC_CE | CC_CVS_66 | CC_PU_330;
#elif(CC_2==CC3)
    USBPD->PORT_CC3 = CC_CE | CC_CVS_66 | CC_PU_330;
#endif
}
...
UINT8 PD_Detect( void )
{
    ...
    {
        USBPD->PORT_CC1 &= ~( CC_CE|CC_CVS_Mask|CC_CMPO );
        USBPD->PORT_CC1 |= ( CC_CE|CC_CVS_22 );
        Delay_Us(2);
        ...
        USBPD->PORT_CC1 &= ~( CC_CE|CC_CVS_Mask|CC_CMPO );
        USBPD->PORT_CC1 |= ( CC_CE|CC_CVS_66 );
#if(CC_2==CC2)
        USBPD->PORT_CC2 &= ~( CC_CE|CC_CVS_Mask|CC_CMPO );
        USBPD->PORT_CC2 |= ( CC_CE|CC_CVS_22 );
        Delay_Us(2);
        ...
        USBPD->PORT_CC2 &= ~( CC_CE|CC_CVS_Mask|CC_CMPO );
        USBPD->PORT_CC2 |= ( CC_CE|CC_CVS_66 );
#elif(CC_2==CC3)
        USBPD->PORT_CC3 &= ~( CC_CE|CC_CVS_Mask|CC_CMPO );
        USBPD->PORT_CC3 |= ( CC_CE|CC_CVS_22 );
        Delay_Us(2);
        ...
        USBPD->PORT_CC3 &= ~( CC_CE|CC_CVS_Mask|CC_CMPO );
        USBPD->PORT_CC3 |= ( CC_CE|CC_CVS_66 );
#endif
...
#elif(CC_2==CC3)
        if((GPIOB->INDR & PIN_CC3) != (uint32_t)Bit_RESET)
        {
           cmp_cc3 |= bCC_CMP_220;
        }
#endif
...
#elif(CC_2==CC3)
            if(USBPD->CONFIG & CC_SEL_3)
            {
                if ((cmp_cc3 & bCC_CMP_220) == bCC_CMP_220)
                {
                    ret=0;
                }
                else
...
#elif(CC_2==CC3)
                if ((((cmp_cc3 & bCC_CMP_22) == bCC_CMP_22) & ((cmp_cc3 & bCC_CMP_66) == 0x00)) == 1)
                {
                    ret = 1;
                }
                if ((cmp_cc3 & bCC_CMP_220) == bCC_CMP_220)
...
#elif(CC_2==CC3)
            if ((((cmp_cc3 & bCC_CMP_66) == bCC_CMP_66) & ((cmp_cc3 & bCC_CMP_220) == 0x00)) == 1)
            {
                if(ret)
                {
                    ret = 0;
